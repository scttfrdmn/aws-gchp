...
`$?`. **The `gchp.slurm` script generated by `scripts/gchp-setup-rundir.sh` already does
this** — it records the start date before the run and exits 0 only if `cap_restart`
advanced *and* a checkpoint was written, so the benign teardown SIGABRT no longer makes
SLURM report the job as `FAILED`. If you hand-write your own job script, copy that
convention.
